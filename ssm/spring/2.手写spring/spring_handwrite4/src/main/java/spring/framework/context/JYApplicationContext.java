...
import spring.framework.aop.JYJdkDynamicAopProxy;
...
//				jyBeanDefinitionMap.put(jyBeanDefinition.getBeanClassName(), jyBeanDefinition); 
	...
	private Object instantiateBean(String beanName, JYBeanDefinition jyBeanDefinition) {
		...
		JYAdvisedSupport config=instantionAopConfig(jyBeanDefinition);
		config.setTargetClass(clazz);
		config.setTarget(instance);
		
		//判断这个对象的类是否属于aop的配置要加强的类，是的话就用代理类，不属于的话就用普通实例就行
		if (config.pointCutMatch()) {
			instance=new JYJdkDynamicAopProxy(config).getProxy();
		}
...
//		factoryBeanObjectCache.put(jyBeanDefinition.getFactoryBeanName(), instance);
	...
	private JYAdvisedSupport instantionAopConfig(JYBeanDefinition jyBeanDefinition) {
		JYAopConfig config=new JYAopConfig();
        ...
        return new JYAdvisedSupport(config);
	}
